...
class MySQL extends AQueryWriter implements QueryWriter
{
	...
	public function createTable( $table )
	{
		...
		$sql   = "CREATE TABLE $table (id INT( 11 ) UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY ( id )) ENGINE = InnoDB ROW_FORMAT=COMPRESSED DEFAULT CHARSET={$encoding} COLLATE={$encoding}_unicode_ci ";
...
class Facade
{
	...
	*/
	...
	public static function store( $bean )
	{
        try
        {
            $id = self::$redbean->store( $bean );
        }
        catch(SQL $e)
        {
            $error_message = $e->getMessage();
            if(strpos($error_message, 'Column not found') > 0 ||
                strpos($error_message, 'doesn\'t exist') > 0)
            {
                self::freeze(false);
                $id = self::$redbean->store( $bean );
            }
